...
for kernel in kernels:
    ...
    plt.tight_layout()
    plt.savefig(f"SVM_boundary_{kernel}.png", dpi=300)
    plt.show()
...
for kernel in kernels:
    ...
    ax.set_title(f"{kernel} stone SVM\nAccuracy: {acc:.2f}")
    plt.tight_layout()
    plt.savefig(f"SVM_confusion_{kernel}.png", dpi=300)
    plt.show()


for kernel, acc in accuracies.items():
    print(f"{kernel}  SVM Accuraccy：{acc:.2f}")
